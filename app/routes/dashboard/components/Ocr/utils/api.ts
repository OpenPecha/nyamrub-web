export async function showOCRContributor(user_id:string ) {
  ...
  console.log()
  try {
      const response = await fetch(`http://localhost:8000/show_ocr_data_to_contributor/${user_id}`);
...
export async function showOCRValidation(user_id:string ) {
  ...
  console.log()
  try {
      const response = await fetch(`http://localhost:8000/show_ocr_data_and_contribution_to_validator/${user_id}`);
      const orcData = await response.json();
      return { status: "success", data: orcData };
  } catch (error) {
      console.error("Error updating contribution:", error);
